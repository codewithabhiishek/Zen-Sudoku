...
test.describe("Offline Play & Syncing", () => {
  test("Local offline moves are not overwritten by an older cloud state upon reconnection", async ({
    page,
    context,
  }) => {
    ...
    db.set("user_test", {
      // We don't know the exact UUID clerk makes, but let's assume it gets captured.
        ...
        return route.fulfill({
          ...
          json: {
            ...
            boardState: {
              ...
              cells: new Array(81)
                .fill({ value: 0, given: false, notes: [] })
                .map((c, i) => (i < 5 ? { ...c, value: 1 } : c)),
              hintsUsed: 0,
            },
          },
        });
