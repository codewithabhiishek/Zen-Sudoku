...
test.describe("Hints, Notes, and Mistakes", () => {
  test("Verify hints, notes, and mistakes persist upon resume and behave correctly", async ({
    page,
  }) => {
    ...
    const emptyIndex2 = cells2.findIndex(
      (c: any, idx: number) => c.value === 0 && idx !== emptyIndex,
    );
    ...
    const persistedStoreStr = await page.evaluate(() =>
      window.localStorage.getItem("sudoku-game-v1"),
    );
