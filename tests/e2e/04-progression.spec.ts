...
test.describe("Level Progression Locking", () => {
  test("Fresh account cannot open Level 10 and Level 2 unlocks after completing Level 1", async ({
    page,
  }) => {
    ...
    await page.reload();
    await expect(page.getByRole("heading", { name: /zen sudoku/i })).toBeVisible({
      timeout: 15000,
    });
