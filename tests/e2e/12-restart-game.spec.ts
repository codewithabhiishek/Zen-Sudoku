...
test.describe("Restart Game (Reset Button)", () => {
  test("Clicking Reset clears board, resets timer/mistakes, and stays on game screen", async ({
    page,
  }) => {
    let saveCount = 0;
    await page.route("**/*.neon.tech/**", async (route) => {
      ...
      if (
        method === "POST" &&
        postData.includes("game_sessions") &&
        postData.includes("board_state")
      ) {
        saveCount++;
      }
    ...
    const emptyIndex3 = cells.findIndex(
      (c: any, idx: number) => c.value === 0 && idx !== emptyIndex1 && idx !== emptyIndex2,
    );
    ...
    const currentStore = await page.evaluate(() =>
      JSON.parse(window.localStorage.getItem("sudoku-game-v1")!),
    );
    ...
    const resetStore = await page.evaluate(() =>
      JSON.parse(window.localStorage.getItem("sudoku-game-v1")!),
    );
    ...
    const resumedStore = await page.evaluate(() =>
      JSON.parse(window.localStorage.getItem("sudoku-game-v1")!),
    );
